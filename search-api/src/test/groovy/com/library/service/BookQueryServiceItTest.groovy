...
class BookQueryServiceItTest extends Specification {
    ...
    @SpringBean
    KakaoBookRepository kakaoBookRepository = Mock(KakaoBookRepository)

    @SpringBean
    NaverBookRepository naverBookRepository = Mock(NaverBookRepository)
    ...
    def "circuit이 open 되면 kakao쪽으로 요청을 한다."() {
        given:
        def keyword = 'HTTP'
        def page = 1
        def size = 10
